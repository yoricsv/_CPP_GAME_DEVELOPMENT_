 ...
 *  OutputDebugString() on Window, and can be funneled by the app with
 ...
 *    "1"       - HIDAPI driver is used
 *
 *  The default is "0" on Window, otherwise the value of SDL_HINT_JOYSTICK_HIDAPI
 */
...
#define SDL_HINT_JOYSTICK_RAWINPUT_CORRELATE_XINPUT   "SDL_JOYSTICK_RAWINPUT_CORRELATE_XINPUT"
  ...
  *          for handling joystick detection and raw input messages on Window
  *
  *  This variable can be set to the following values:
 ...
 *
 *  On some platforms, currently Window and X11, OpenGL drivers may support
...
*  Support for this hint is currently available only in the pthread, Window, and PSP backend.
*
 ...
 *  power and CPU time.  This hint is only used on Window.
 *
...
*
*  SDL has EGL and OpenGL ES2 support on Window via the ANGLE project. It
...
/**
 * \brief Tell SDL not to name threads on Window with the 0x406D1388 Exception.
 *        The 0x406D1388 Exception is a trick used to inform Visual Studio of a
 *        thread's name, but it tends to cause problems with other debuggers,
 *        and the .NET runtime. Note that SDL 2.0.6 and later will still use
 *        the (safer) SetThreadDescription API, introduced in the Window 10
...
/**
 * \brief Force SDL to use Critical Sections for mutexes on Window.
 *        On Window 7 and newer, Slim Reader/Writer Locks are available.
...
/**
 * \brief Force SDL to use Kernel Semaphores on Window.
 *        Kernel Semaphores are inter-process and require a context
 *        switch on every interaction. On Window 8 and newer, the
...
/**
 * \brief A variable to specify custom icon resource id from RC file on Window platform
 */
#define SDL_HINT_WINDOWS_INTRESOURCE_ICON       "SDL_WINDOWS_INTRESOURCE_ICON"
#define SDL_HINT_WINDOWS_INTRESOURCE_ICON_SMALL "SDL_WINDOWS_INTRESOURCE_ICON_SMALL"

/**
 *  \brief Tell SDL not to generate window-close events for Alt+F4 on Window.
 *
 * The variable can be set to the following values:
...
/**
 * \brief Use the D3D9Ex API introduced in Window Vista, instead of normal D3D9.
...
/** \brief Allows back-button-press events on Window Phone to be marked as handled
 *
 *  Window Phone devices typically feature a Back button.  When pressed,
 *  the OS will emit back-button-press events, which apps are expected to
 *  handle in an appropriate manner.  If apps do not explicitly mark these
 *  events as 'Handled', then the OS will invoke its default behavior for
 *  unhandled back-button-press events, which on Window Phone 8 and 8.1 is to
 ...
 *  the app.
 *
 *  Internally, Window Phone sends back button events as parameters to
 ...
 *  the app).
 *
 *  SDL registers its own back-button-press callback with the Window Phone
 ...
 *
 *  More details on back button behavior in Window Phone apps can be found
 ...
 *
 *  Network-enabled WinRT apps must include a privacy policy.  On Window 8, 8.1, and RT,
 *  Microsoft mandates that this policy be available via the Window Settings charm.
 ...
 *  users.  On Window 8, 8.1, and RT, Microsoft mandates that this policy be
 *  be available in the Window Settings charm, as accessed from within the app.
 ...
 *
 *  Please note that on Window Phone, Microsoft does not provide standard UI
