...
D3D11_CreateDeviceResources(SDL_Renderer * renderer)
{
    ...
    result = D3D11CreateDeviceFunc(
        ...
        D3D11_SDK_VERSION, /* Always set this to D3D11_SDK_VERSION for Window Store apps. */
...
D3D11_CreateSwapChain(SDL_Renderer * renderer, int w, int h)
{
    ...
    /* TODO, WinRT: see if Win 8.x DXGI_SWAP_CHAIN_DESC1 settings are available on Window Phone 8.1, and if there's any advantage to having them on */
#else
    if (usingXAML) {
    ...
    swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL; /* All Window Store apps must use this SwapEffect. */
#endif
...
#else
        SDL_SetError(SDL_COMPOSE_ERROR("XAML support is not yet available for Window Phone"));
        result = E_FAIL;
        goto done;
#endif
    } else {
...
D3D11_CreateWindowSizeDependentResources(SDL_Renderer * renderer)
{
    ...
    if (data->swapChain) {
        /* IDXGISwapChain::ResizeBuffers is not available on Window Phone 8. */
     ...
     * on Window Phone 8.0, nor is it supported there.
     *
     * IDXGISwapChain1::SetRotation does seem to be available on Window Phone 8.1,
     ...
     * Window Phone 8.1 (it wasn't on Window Phone 8.0), but I'm not 100% sure of this.
...
D3D11_UpdateViewport(SDL_Renderer * renderer)
{
     ...
     * a landscape mode, for all Window 8/RT devices, or a portrait mode,
     * for Window Phone devices.
     */
...
D3D11_RenderPresent(SDL_Renderer * renderer)
{
     ...
     * This option is not available on Window Phone 8, to note.
     */
...
#if WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP
    /* VSync is required in Window Phone, at least for Win Phone 8.0 and 8.1.
...
#ifdef __WIN32__
/* This function needs to always exist on Window, for the Dynamic API. */
ID3D11Device *
