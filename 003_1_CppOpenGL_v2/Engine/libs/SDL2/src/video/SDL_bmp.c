...
/*
   Code to load and save surfaces in Window BMP format.

   Why support BMP format?  Well, it's a native format for Window, and
...
SDL_LoadBMP_RW(SDL_RWops * src, int freesrc)
{
    ...
    if (SDL_strncmp(magic, "BM", 2) != 0) {
        SDL_SetError("File is not a Window BMP file");
        was_error = SDL_TRUE;
        goto done;
    }
