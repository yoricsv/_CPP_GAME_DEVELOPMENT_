...
{
#ifdef __WIN32__
    /* The Window driver is taking care of acks */
#else
    if ((data[1] & 0x30) == 0x30) {
...
/*
 * Xbox One S with firmware 4.8.1923 uses a 17 byte packet with BACK button in byte 16 and the GUIDE button in a separate packet (on Window), or in byte 15 (on Linux)
...
#ifdef __WIN32__
            /* The Window driver is taking care of negotiation */
            SetInitState(ctx, XBOX_ONE_INIT_STATE_COMPLETE);
#else
